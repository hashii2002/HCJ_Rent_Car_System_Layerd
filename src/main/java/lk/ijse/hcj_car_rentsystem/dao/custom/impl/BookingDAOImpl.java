...
//    }
    ...
    public BookingDTO searchBookingByCustomerID(int cusId) throws SQLException {
        ResultSet rs = CrudUtil.execute("SELECT * FROM bookings WHERE customer_id=?", cusId);
...
//    }
    ...
    public Booking search(String id) throws SQLException {
        ResultSet rs = CrudUtil.execute("SELECT * FROM bookings WHERE booking_id=?", id);

        if (rs.next()) {
            return new Booking(
                    rs.getInt("booking_id"),
                    rs.getInt("customer_id"),
                    0, // vehicle_id (meeka join table ekakin ena nisa 0 damma)
                    0, // driver_id
                    rs.getString("booking_date"),
                    rs.getString("start_date"),
                    rs.getString("end_date"),
